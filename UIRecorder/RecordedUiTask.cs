...
namespace JupiterUIRecorder
{
    ...
    class GeneratePyCode
    {
        ...
        public static string DoubleClick(RecordedUiTask uiTask, string elemName)
        {
            string codeLine = $"    action = ActionChains(self.driver)\n" +
                            $"    action.double_click(winElem_{elemName}).perform()\n";
        ...
        {
            string codeLine = $"    action = ActionChains(self.driver)\n" +
                            $"    action.move_to_element(winElem_{elemName})\n" +
